    ...
    >
      {!file ? (
        ...
        <div className="flex flex-col h-[70vh]">
          <div className="flex justify-between items-center p-2 bg-gray-100 dark:bg-gray-800 rounded-t-lg border-b dark:border-gray-700">
          ...
          <div className="flex-grow p-4 overflow-y-auto bg-white dark:bg-gray-900">
            {messages.map((msg, index) => (
              <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} mb-4`}>
                <div className={`max-w-prose p-3 rounded-lg ${msg.role === 'user' ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>
                  {msg.text}
                </div>
              </div>
            ))}
            {isLoading && (
              <div className="flex justify-start mb-4">
                  <div className="max-w-prose p-3 rounded-lg bg-gray-200 dark:bg-gray-700">
          ...
          <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-b-lg border-t dark:border-gray-700">
            ...
            <div className="flex gap-2">
              ...
              />
              <button onClick={handleSendMessage} disabled={isLoading || !currentMessage} className="px-6 py-2 font-semibold text-white bg-brand-primary rounded-lg disabled:bg-gray-400">Send</button>
            </div>
          </div>
        </div>
      )}
