        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea value={from} onChange={e => setFrom(e.target.value)} rows={4} className="w-full p-2 border rounded-md bg-slate-800 border-slate-600"></textarea>
                </div>
                <div>
                    <label className="block font-semibold mb-1">To</label>
                    <textarea value={to} onChange={e => setTo(e.target.value)} rows={4} className="w-full p-2 border rounded-md bg-slate-800 border-slate-600"></textarea>
                </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label className="block font-semibold mb-1">Invoice Number</label>
                    <input type="text" value={invoiceNumber} onChange={e => setInvoiceNumber(e.target.value)} className="w-full p-2 border rounded-md bg-slate-800 border-slate-600" />
                </div>
                <div>
                    <label className="block font-semibold mb-1">Date</label>
                    <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full p-2 border rounded-md bg-slate-800 border-slate-600" />
                </div>
            </div>

            <h3 className="font-bold text-lg mt-6 border-t pt-4 border-slate-700">Items</h3>
            {items.map((item, index) => (
                <div key={index} className="flex items-center gap-2">
                    <input type="text" placeholder="Description" value={item.description} onChange={e => handleItemChange(index, 'description', e.target.value)} className="w-1/2 p-2 border rounded-md bg-slate-800 border-slate-600" />
                    <input type="number" placeholder="Quantity" value={item.quantity} onChange={e => handleItemChange(index, 'quantity', parseFloat(e.target.value))} className="w-1/6 p-2 border rounded-md bg-slate-800 border-slate-600" />
                    <input type="number" placeholder="Price" value={item.price} onChange={e => handleItemChange(index, 'price', parseFloat(e.target.value))} className="w-1/6 p-2 border rounded-md bg-slate-800 border-slate-600" />
            ...
            <button onClick={addItem} className="px-4 py-2 bg-slate-700 rounded-md">Add Item</button>
