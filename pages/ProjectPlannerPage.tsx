...
import { PlusIcon } from '@heroicons/react/24/solid'; // Assuming heroicons are available or create a simple plus icon

const PlusIconComponent: React.FC<{ className?: string }> = (props) => (
            ...
            <div className="space-y-4 mb-6">
                ...
                <div className="flex gap-2">
                    <input
                        ...
                        className="flex-grow p-2 border rounded-md bg-slate-800 border-slate-600"
                    />
                    <button onClick={handleAddTask} className="px-4 py-2 font-semibold text-slate-900 bg-primary rounded-lg">Add Task</button>
                </div>
            </div>
                            ...
                            <div className="space-y-3 min-h-[40vh]">
                                {column.items.map(task => (
                                    <div
                                        ...
                                        className="p-3 bg-slate-700 rounded-md shadow-md cursor-grab active:cursor-grabbing"
                                    >
                                        {task.content}
                                    </div>
                                ))}
