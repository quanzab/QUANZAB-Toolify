...
const TaskItem: React.FC<TaskItemProps> = ({ task, onToggle, onDelete, onUpdateDueDate }) => {
  ...
  return (
    <li className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 bg-slate-800 rounded-lg border border-slate-700 transition-colors duration-300 hover:bg-slate-700/50 gap-2">
      <div className="flex items-center gap-3 flex-grow min-w-0">
        <input
          type="checkbox"
          checked={task.completed}
          onChange={() => onToggle(task.id)}
          className="h-5 w-5 rounded bg-slate-700 border-slate-600 text-primary focus:ring-primary flex-shrink-0 mt-1 sm:mt-0"
        />
        <span className={`font-medium text-gray-300 transition-all break-words ${task.completed ? 'line-through text-slate-500' : ''}`}>
          {task.text}
        </span>
      </div>
      <div className="flex items-center gap-3 flex-shrink-0 w-full sm:w-auto justify-end pl-8 sm:pl-0">
        <div className="flex items-center gap-2 relative">
            <label className={`cursor-pointer text-xs font-semibold px-2 py-1 rounded-md ${isOverdue ? 'bg-red-500/20 text-red-300' : 'bg-slate-700 text-slate-300'}`}>
