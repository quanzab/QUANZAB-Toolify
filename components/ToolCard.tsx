...
const ToolCard: React.FC<ToolCardProps> = ({ tool, accentColor }) => {
  ...
  const tagColors: { [key: string]: string } = {
    ...
    'Popular': 'bg-fuchsia-500/20 text-fuchsia-300 border-fuchsia-500/50',
    'PRO': 'bg-indigo-500/20 text-indigo-300 border-indigo-500/50',
  };
  ...
  return (
    <Link to={path} style={cardStyle} className="block group h-full relative [perspective:1000px]">
      ...
      <div className="relative bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700 transition-all duration-300 ease-in-out h-full transform group-hover:-translate-y-1">
        <div className="flex justify-between items-start mb-4">
            ...
            <div className="flex gap-1.5 flex-wrap justify-end flex-shrink-0 pl-4">
                {displayTags.map(tag => (
                    <span key={tag} className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs font-semibold rounded-full border ${tagColors[tag] || 'bg-slate-700'}`}>
